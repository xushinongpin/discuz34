...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_qqnews&pmod=released';
function return_seo($data,$url)
{
	$data_arr=json_decode($data,true);
	$restr=lang('plugin/csdn123com_qqnews', 'baiduseo_url') . $url . '<br>';
	$restr=$restr . lang('plugin/csdn123com_qqnews', 'baiduseo_success') . $data_arr['success'] . '<br>';
	$restr=$restr . lang('plugin/csdn123com_qqnews', 'baiduseo_remain') . $data_arr['remain'] . '<br>';
	$restr=$restr . '<span onClick="document.getElementById(\'showbaiduseoinfo\').style.display=\'block\'" style="cursor:pointer;color:red">' . lang('plugin/csdn123com_qqnews', 'baiduseo_info') . '</span><br>';
...
if ($_GET['formhash'] == FORMHASH && empty($_GET['clears_all']) == false && $_GET['clears_all'] == 'yes') {
	...
	DB::delete('csdn123qqnews_news', 'tid_aid>0');
	...
	{
		$count_num = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123qqnews_news') . ' WHERE tid_aid>0');		
	} else {		
	...
	if(empty($_G['cache']['plugin']['csdn123com_qqnews']['csdn123_seo']))
	{
		cpmsg('csdn123com_qqnews:baidu_seo_error', '', 'error');
	} else {
		$csdn123_seo=$_G['cache']['plugin']['csdn123com_qqnews']['csdn123_seo'];
	}
	$post_seo_url_RS=DB::fetch_first("SELECT * FROM " . DB::table('csdn123qqnews_news') . " WHERE tid_aid>0 AND ID>" . $id . " ORDER BY ID ASC LIMIT 1");
	...
	{	
		...
		$statusStr = lang('plugin/csdn123com_qqnews', 'baiduseo_status');		
	...
	if (empty($_GET['idarray'])) {
		cpmsg('csdn123com_qqnews:select_empty', '' , 'error');
	}
	if(empty($_GET['seldelete'])==false)
	{
		...
		DB::delete('csdn123qqnews_news', 'ID in (' . $idstr . ')');
		cpmsg('csdn123com_qqnews:succeed', $server_url . '&page=' . $_GET['page'], 'succeed');
	...
	if(empty($_GET['selpostbaidu'])==false)
	{
		...
		$csdn123_seo=trim($csdn123_seo);		
		$post_seo_url_RS=DB::fetch_first("SELECT * FROM " . DB::table('csdn123qqnews_news') . " WHERE tid_aid>0 AND ID=" . $ID);
		$post_seo_url=preview_url($post_seo_url_RS['forum_portal'],$post_seo_url_RS['tid_aid']);
		if(function_exists('curl_init') && function_exists('curl_exec')) {
	...
	$page=max(1,$page);
	$newsCount = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123qqnews_news') . ' WHERE tid_aid>0');
	if($newsCount>0)
	{
		$TotalNumber = lang('plugin/csdn123com_qqnews', 'total_number');
		...
		$news_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123qqnews_news') . ' WHERE tid_aid>0 ORDER BY send_datetime DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template("csdn123com_qqnews:released");
