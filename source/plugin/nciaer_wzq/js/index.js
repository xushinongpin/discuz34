...
var fiveChess = {
	...
	init: function () {
		this.chessBoardHtml = jQuery("div.chessboard").html();
		var _fiveChess = this;
		//右侧操作按钮
		jQuery(".operating-panel a").click(function (event) {
	...
	//jQuery("#result_info").html("胜率：" + (this.winGames * 100 / this.totalGames | 0) + "%");
},
//重置棋盘
resetChessBoard: function () {
	jQuery("div.chessboard").html(this.chessBoardHtml);
	jQuery("#result_tips").html("");
	this.isGameOver = false;
	this.isPlayerTurn = jQuery("#first_move_btn").hasClass("selected");
	//初始化棋子状态
	var i, j;
	for (i = 0; i < 15; i++) {
		this.chessArr[i] = [];
	...
	//player下棋事件
	var _fiveChess = this;
	jQuery("div.chessboard div").click(function () {
		...
		if (_fiveChess.chessArr[i][j] === _fiveChess.NO_CHESS) {
			...
			if (i === 0 && j === 0) {
				jQuery(this).removeClass("hover-up-left");
			} else if (i === 0 && j === 14) {
				jQuery(this).removeClass("hover-up-right");
			} else if (i === 14 && j === 0) {
				jQuery(this).removeClass("hover-down-left");
			} else if (i === 14 && j === 14) {
				jQuery(this).removeClass("hover-down-right");
			} else if (i === 0) {
				jQuery(this).removeClass("hover-up");
			} else if (i === 14) {
				jQuery(this).removeClass("hover-down");
			} else if (j === 0) {
				jQuery(this).removeClass("hover-left");
			} else if (j === 14) {
				jQuery(this).removeClass("hover-right");
			} else {
...
//鼠标在棋盘上移动效果
jQuery("div.chessboard div").hover(
function () {
...
if (_fiveChess.chessArr[i][j] === _fiveChess.NO_CHESS) {
if (i === 0 && j === 0) {
jQuery(this).addClass("hover-up-left");
}
else if (i === 0 && j === 14) {
jQuery(this).addClass("hover-up-right");
}
else if (i === 14 && j === 0) {
jQuery(this).addClass("hover-down-left");
}
else if (i === 14 && j === 14) {
jQuery(this).addClass("hover-down-right");
}
else if (i === 0) {
jQuery(this).addClass("hover-up");
}
else if (i === 14) {
jQuery(this).addClass("hover-down");
}
else if (j === 0) {
jQuery(this).addClass("hover-left");
}
else if (j === 14) {
jQuery(this).addClass("hover-right");
}
else {
...
jQuery(".operating-panel p a").addClass("disable");
jQuery("#replay_btn").html("重玩");
},
gameOver: function () {
this.isGameStart = false;
jQuery(".operating-panel a").removeClass("disable");
jQuery("#replay_btn").html("开始");
jQuery("#result_info").html("胜率：" + (this.winGames * 100 / this.totalGames | 0) + "%");
},
//下棋 i行，j列，color颜色
...
jQuery("div.chessboard div." + color + "-last").addClass(color).removeClass(color + "-last");
jQuery("div.chessboard div:eq(" + (i * 15 + j) + ")").addClass(color + "-last");
}
else {
jQuery("div.chessboard div:eq(" + (i * 15 + j) + ")").addClass(color);
}
},
//玩家是否取胜
...
if (isPlayerWin) {
jQuery("#result_tips").html("恭喜你获胜！");
}
else {
jQuery("#result_tips").html("很遗憾，你输了！");
}
...
jQuery("div.chessboard div." + this.AIPlayer + "-last").addClass(this.AIPlayer).removeClass(this.AIPlayer + "-last");
//x方向
lineChess[0] = [i];
lineChess[1] = [j];
for (m = j - 1; m >= 0; m--) {
...
jQuery("div.chessboard div:eq(" + pos + ")").removeClass(color).addClass(color + "-last");
},
