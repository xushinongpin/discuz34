...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_zaker&pmod=adminVest&subpmod=regvest&formhash=' . FORMHASH;
if ($_GET['formhash'] == FORMHASH && empty($_GET['adduser']) == false && $_GET['adduser'] == 'yes') {
	...
	{
		cpmsg("csdn123com_zaker:reg_err", '', "error");
	}
	...
	foreach ($regData as $regValue) {
		...
		DB::insert('csdn123zaker_reguser', $addUser);
		
	}
	$user_list = DB::fetch_all('select * from ' . DB::table('csdn123zaker_reguser') . ' order by uid desc');
	...
	cpmsg("csdn123com_zaker:registered_successfully", $server_url, "succeed");
	...
	include template('csdn123com_zaker:regvest');
