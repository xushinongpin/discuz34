...
if ($_GET['formhash'] == FORMHASH && empty($_GET['canceldel']) == false) {
	...
	if (empty($_GET['selcancel']) == false) {
		$idstr = implode(',', $_GET['idarray']);
		$idstr = daddslashes($idstr);
		DB::update('csdn123zaker_news', array('del' => 0), 'ID in (' . $idstr . ')');
		cpmsg('csdn123com_zaker:succeed', $server_url . '&page=' . $_GET['page'], 'succeed');
	}
	...
	$page=max(1,$page);
	$newsCount = DB::result_first("SELECT COUNT(*) FROM " . DB::table('csdn123zaker_news') . " WHERE tid_aid=0 and del=1");
	if($newsCount>0)
	{
		...
		$news_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123zaker_news') . ' WHERE tid_aid=0 and del=1 ORDER BY ID DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template("csdn123com_zaker:recycle");	
