...
$server_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_zaker&pmod=url';
...
if($_GET['formhash'] == FORMHASH && !empty($_GET['csdn123_agrs']) && $_GET['csdn123_agrs']=='yes')
{
	...
	{
		cpmsg("csdn123com_zaker:content_url_error","","error");
	} else {
	...
	$chk = DB::fetch_first("SELECT * FROM " . DB::table('csdn123zaker_news') . " WHERE source_link='" . daddslashes($content_url) . "' LIMIT 1");
	if (count($chk) == 0)
	{
		$newsArr=array();
		...
		$ID = DB::insert('csdn123zaker_news', $newsArr,true);
	} else {
		$ID = $chk["ID"];
	}
	if($chk['tid_aid']>0)
	{
		echo lang('plugin/csdn123com_zaker', 'collection_success') . '<br><br>' . preview_url($chk['forum_portal'],$chk['tid_aid']);
		exit;
	}	
	...
	if($recode=='ok')
	{
		$succeed_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_zaker&pmod=released';
	...
	include template('csdn123com_zaker:url');
