...
$server_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_zaker&pmod=fixedTime';
...
if($_GET['formhash'] == FORMHASH && !empty($_GET['csdn123_agrs']) && $_GET['csdn123_agrs']=='yes')
{
	...
	{
		DB::insert('csdn123zaker_cron', $cronArr);
	} else {
		$news_id=intval($_GET['news_id']);
		DB::update('csdn123zaker_cron',$cronArr,'ID=' . $news_id);
	}
	$succeed_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_zaker&pmod=fixedTime';
	cpmsg("csdn123com_zaker:succeed",$succeed_url,"succeed");
	...
	$id = intval($_GET['del']);
	DB::delete('csdn123zaker_cron','ID=' . $id);
	cpmsg("csdn123com_zaker:succeed",$server_url,"succeed");
	...
	$update_rs=DB::fetch_first("SELECT * FROM " . DB::table('csdn123zaker_cron') . " WHERE ID=" . $update_id);
	...
	include template('csdn123com_zaker:fixedTimeUpdate');

} elseif(!empty($_GET['clears_all']) && $_GET['clears_all']=='yes' && $_GET['formhash'] == FORMHASH) {
	
	DB::delete('csdn123zaker_cron','ID>0');
	cpmsg('csdn123com_zaker:succeed', $server_url, 'succeed');
	...
	include template('csdn123com_zaker:fixedTimeAdd');
	...
	$hzw_startcron = $_G['cache']['plugin']['csdn123com_zaker']['csdn123_dingshi'];
	if($hzw_startcron != 1)
	{
		echo '<div style="text-align:center;margin:64px;"><a href="?action=plugins&operation=config&do=' . $pluginid . '" style="font-size:24px;color:red;">' . lang('plugin/csdn123com_zaker', 'open_timing_acquisition') . '</a></div>';
		...
		$pendingUrl='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_zaker&pmod=pending';
		$cronList=DB::fetch_all("SELECT * FROM " . DB::table('csdn123zaker_cron') . " ORDER BY catchtime DESC");
		$pending_count = lang('plugin/csdn123com_zaker', 'pending_count');
		...
		include template('csdn123com_zaker:fixedTime');
