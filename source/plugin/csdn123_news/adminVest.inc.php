...
if ($_GET['formhash'] == FORMHASH && empty($_GET['deluser']) == false && $_GET['deluser'] == 'yes') {
	...
	{
		DB::delete('csdn123zd_reguser', 'uid>0');
	...
	foreach ($uidarray as $uid) {
		DB::delete('csdn123zd_reguser', 'uid=' . intval($uid));
	}
	include template('csdn123_news:delete_vest_list');
	...
	include template('csdn123_news:uc_avatar');
	
} elseif($_GET['formhash'] == FORMHASH && !empty($_GET['subpmod']) && $_GET['subpmod']=='regvest') {
	
	require './source/plugin/csdn123_news/regvest.inc.php';	

} elseif($_GET['formhash'] == FORMHASH && !empty($_GET['subpmod']) && $_GET['subpmod']=='importVest') {
	
	require './source/plugin/csdn123_news/importVest.inc.php';
	
} elseif($_GET['formhash'] == FORMHASH && !empty($_GET['subpmod']) && $_GET['subpmod']=='exportVest') {
	
	require './source/plugin/csdn123_news/exportVest.inc.php';
	
} elseif ($_GET['formhash'] == FORMHASH && empty($_GET['output']) == false && $_GET['output'] == 'yes') {
	
	
	$uidarray = DB::fetch_all('select uid from ' . DB::table('csdn123zd_reguser') . ' order by uid desc');
	foreach ($uidarray as $uidvalue) {
		$uidstr = $uidvalue['uid'] . ',' . $uidstr;
	}
	$uidstr = substr($uidstr, 0, -1);
	include template('csdn123_news:output_user_list');
	
	
} else {
	
	$VestCount = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123zd_reguser'));
	if($VestCount>0)
	{
		...
		$user_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123zd_reguser') . ' ORDER BY uid DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template('csdn123_news:adminVest');
